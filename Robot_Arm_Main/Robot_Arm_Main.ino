...
void loop() {
  ...
  ******************************************************************************************************************/
  ...
  /*****************************************************************************************************************
   You can move the robotic arm servos independently using the following commands:
    
    moveBase(int stepDelay, int vBase);
    moveShoulder(int stepDelay, int vShoulder);
    moveElbow(int stepDelay, int vElbow);
    moveWrist_Ver(int stepDelay, int vWrist_Ver);
    moveWrist_Rot(int stepDelay, int vWrist_rot);
    moveGripper(int stepDelay, int vgripper);

   You can print the current position of each servo using:
  ...
  ******************************************************************************************************************/
  ...
  moveShoulder(20,180); // moves the shoulder to 180 degrees

  printPosition(); // shows current position of each servo

  homePosition(); // robotic arm returns to home position
}